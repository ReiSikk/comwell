...
function WidgetInput({ inputText, overlayID }) {

  const { updateOverlayState, overlayState }= useContext(HotelsContext);


  return (
 <div className={styles.input_wrapper} onClick={() => updateOverlayState({ ...overlayState,  overlayToShow: overlayID, showOverlay: !overlayState.showOverlay })} >
