...
const SignUpOverlay = ({ closeSignUpOverlay }) => {
  ...
  async function handleSubmit(event) {
    ...
    if (termsAccepted && passwordData.signupPassword === passwordData.confirmPassword) {
      //sending the request
      callSignUpBackend();
    } else {
      if (!termsAccepted) {
      ...
      <div className={styles.overlayContent}>
        <div className={styles.button_container}>
          <button className={styles.close_button} onClick={closeSignUpOverlay}>
            <svg className={styles.close_icon} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 16">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles.condtitions_container}>
            <div className="container">
              <CheckboxWithText
                label="Accept terms and conditions for Comwell Club *"
                id="conditions-checkbox"
                onCheckboxChange={handleAcceptTermsChange}
                errorMessage={showTermsError && !termsAccepted ? "Please accept the terms and conditions." : null}
              />
              <CheckboxWithText
                label="I would like to be updated on current member offers, Comwell Club surprises and other recommendations personalized to me. I can unsubscribe again at any time. "
                id="newsletter-checkbox"
                onCheckboxChange={handleReceiveNewsletterChange}
              />
            </div>
          </div>
